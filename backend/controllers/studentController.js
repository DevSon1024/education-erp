...
const getStudentById = asyncHandler(async (req, res) => {
  ...
  const student = await Student.findById(req.params.id).populate(
    "course",
    "name admissionFees registrationFees"
  );
...
const confirmStudentRegistration = asyncHandler(async (req, res) => {
  ...
  // FIXED: Changed message to match Frontend expectation
...
const resetStudentLogin = asyncHandler(async (req, res) => {
  ...
  if (!student.userId) {
    ...
    res.status(400);
    throw new Error(
      "Student is not registered yet. Please confirm registration first."
    );
  }
  ...
  if (student.mobileStudent) {
    const msg = `Dear ${
      student.firstName
    }, your login details have been updated. User ID: ${
      user.username
    }, Password: ${password || "(Unchanged)"}. Smart Institute.`;
    sendSMS(student.mobileStudent, msg).catch((err) =>
      console.error("Reset Login SMS failed", err)
    );
  }
