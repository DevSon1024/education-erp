...
import { fetchStudentById, updateStudent, resetStatus } from '../../../features/student/studentSlice';
import { fetchBatches } from '../../../features/master/masterSlice';
import { fetchEmployees } from '../../../features/employee/employeeSlice';
import { useNavigate, useParams } from 'react-router-dom';
import { toast } from 'react-toastify';
import { 
    ChevronRight, Save, X, UserCheck, 
} from 'lucide-react';
...
const StudentUpdate = () => {
    ...
    // Redux
    const { currentStudent, isSuccess, isLoading, message } = useSelector((state) => state.students);
    ...
    if (!currentStudent) return <div className="p-10 text-center">Loading Student Data...</div>;
        ...
        <div className="bg-gray-100 min-h-screen p-6 font-sans">
            <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
                <div className="bg-gradient-to-r from-orange-600 to-red-600 p-4 flex justify-between items-center text-white shadow-md">
                    <h1 className="text-xl font-bold flex items-center gap-2"><UserCheck size={24}/> Update Student Details</h1>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-8">
                    ...
                    <div className="grid grid-cols-12 gap-5 animate-fade-in-up">
                        ...
                        <div className="col-span-12 md:col-span-4">
                            ...
                            <input {...register('aadharCard', {minLength:12})} className="input bg-gray-100 cursor-not-allowed" disabled title="Cannot change unique ID"/>
                        </div>
                            ...
                            <label className="relative cursor-pointer group">
                                <div className="w-24 h-24 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden bg-gray-50 group-hover:border-blue-500 transition">
                                    {previewImage || currentStudent?.studentPhoto ? (
                                        <img 
                                            src={previewImage || `http://localhost:5000/${currentStudent.studentPhoto}`} 
                                            className="w-full h-full object-cover"
                                            onError={(e) => {e.target.src = 'https://via.placeholder.com/150'}}
                                        /> 
                                    ) : (
                                        <div className="text-gray-400 text-xs text-center p-1">No Photo</div>
                                    )}
                                </div>
                                <input type="file" className="hidden" onChange={handleImageChange} accept="image/*"/>
                                <span className="block text-center text-xs text-blue-600 font-bold mt-1 group-hover:underline">Change Photo</span>
                            </label>
                        </div>
                        ...
                        <div className="col-span-12">
                            ...
                            <input list="eduOptions" {...register('education')} className="input" />
                        ...
                        <div className="col-span-12 md:col-span-4">
                         ...
                         <div className="col-span-12 mt-6 border-t pt-6">
                            <h3 className="text-lg font-bold text-gray-700 mb-4 bg-gray-50 p-2 border-l-4 border-orange-500">
                                Update Batch <span className="text-xs font-normal text-red-500 ml-2">(Course cannot be changed)</span>
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select {...register('batch')} className="input">
                                        ...
                                        {batches
                                            .filter(b => b.course === watchCourseId || b.courses?.some(c => (c._id || c) === watchCourseId))
                                            .map(b => (
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">Select a new batch to move the student.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="col-span-12 flex justify-end gap-4 mt-8 border-t pt-4">
                            <button type="button" onClick={() => navigate('/master/student')} className="btn-secondary">
                                Cancel
                            </button>
                            <button type="submit" disabled={isLoading} className="bg-orange-600 text-white px-8 py-2 rounded font-bold hover:bg-orange-700 flex items-center gap-2 shadow opacity-90 hover:opacity-100 transition">
                                <Save size={18}/> {isLoading ? 'Updating...' : 'Update Student Details'}
                            </button>
                        </div>
                    </div>
                </form>
